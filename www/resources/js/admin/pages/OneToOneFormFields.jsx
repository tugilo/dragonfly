...
const NOTES_LABEL = 'この回で話した内容・実施後記録（1レコード＝1回の1 to 1）';
const NOTES_HELPER_CREATE =
    '同一相手とも定期的に実施する前提で、予定・実施ごとに行が分かれます。この欄に当該回の議題・話したこと・次アクションなどを書きます。Zoom や AI の要約をそのまま貼り付け（コピペ）しても構いません。';
const NOTES_HELPER_EDIT =
    'その回の記録の本体（実施後記録）です。Zoom や AI の要約をそのまま貼り付け（コピペ）して保存できます。Markdown 記法に対応し、保存後は下にプレビュー表示されます。追記だけ時系列で分けたい場合は下の「履歴メモ（contact_memos）」も利用できます。';

...
- **合意事項:** **Google Workspace / GAS 案は採用しない**（外部アクセス・セキュリティの懸念）。代わりに、既存 Windows サーバーの **Hyper-V 上に Linux Web サーバーを構築**し、社内ネットワーク（Wi-Fi / VPN）経由で利用する **社内完結型 Web システム**で進める。第一段階は PDF 入力自動化、第二段階以降で Excel 帳票の Web化を検討。
- **次アクション:** 次廣が **提案書と簡易モックを無償作成**し、実現可能性・進め方・費用感を提示。**2026-07-05** に DB 定義書・Excel サンプル・PDF 注文書（1パターン）を [`docs/proposals/木村さん用/`](../../proposals/木村さん用/README.md) へ受領。追加 PDF パターンは確認中。
...
- **提案書:** [PDF注文書入力支援システム 提案書（たたき台）](../../proposals/kimura_kokuhosha_pdf_order_input_proposal.md) — 2026-07-05 作成。
...
- **Google Workspace / GAS 案は不採用。** 第2回121で **社内ローカルサーバー（Hyper-V 上 Web）** 展開に合意。
...
- **ヒアリングで見る点:** 業務フロー、既存 VB 画面、Oracle テーブル、Excel ODBC の使い方、社内利用者数、保守担当の有無。
- **提案パターン:**
  - 社内 Web 入力支援 + Oracle 連携（第2回121合意・本命）
  - 現行システム延命＋データ連携
  - Web フロント追加
  - 段階的リプレイス

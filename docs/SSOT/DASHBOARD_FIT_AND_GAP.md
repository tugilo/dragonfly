...
| 3 | リファーラル件数（今月）— *紹介メモ（introduction）件数をプロキシ* | 12 | BO含む |
...
| KPI③ リファーラル件数（今月） | owner が今月記録した紹介メモ（introduction）件数（UI ラベル。集計定義は [DASHBOARD_DATA_SSOT.md](DASHBOARD_DATA_SSOT.md)） | `contact_memos` | `memo_type=introduction`・`created_at` 当月 |
...
| Tasks | **実装済** | `GET /api/dashboard/tasks`。kind 別スタイルはモックに準拠。**DASHBOARD-P7-2:** 2 件目「メモ追加」は **`/members/:id/show` deep link**（**有効**。SSOT と一致）。**DASHBOARD-TASKS-ALIGNMENT-P1/P2:** 見出し「優先アクション」、**`meeting_follow_up`** は **直近開催済み例会の例会メモ未記録時のみ**（`contact_memos`・一覧 has_memo と同型）。**STALE-WORKSPACE-SCOPE-P1 / P2:** **`stale_follow`** は **`getSummaryLiteBatch(..., null)`**・peer は **owner 以外全員**（`stalePeerMemberIds`）。**resolveWorkspaceIdForUser** を stale に渡すのは **未実装**（**`members.workspace_id` は列追加済・MEMBERS-WORKSPACE-BACKFILL-P1** だが **Dashboard が peer をまだ全会員のまま**）。**MEMBER-SUMMARY-WORKSPACE-NULL-P1:** Members 一覧 `summary_lite`（`workspace_id` 指定時）の Query は **DATA_MODEL §5.1** の **OR NULL** に整合。 |

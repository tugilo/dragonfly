...
### 2.1 BNI Givers Gain とリファーラル提案（COMMON 参照）

BNI 理念 **Givers Gain**（与えるものは与えられる）に対し、リファーラル提案機能族は次を製品既定とする（詳細は [REFERRAL_SUGGESTION_COMMON.md](REFERRAL_SUGGESTION_COMMON.md) **§0.8.7**）。

| 場面 | `introductions` の典型 | 意味 |
|------|------------------------|------|
| **自分の 121 で紹介すると書いた**（経路 ①） | `from` = 記録者（紹介を出す側）、`to` = 紹介先メンバー等 | **自分が与える**約束の履行 |
| **他メンバー A のネットワークの B**（経路 ②） | `from` = **A（つなぎ手）**, `to` = **依頼者 C** | **A が B を C に紹介した**実績。C は to。B は `note` 等 |

**採用しない:** 依頼者 C を `from` にし、A のネットワークの B を横取りする登録。B への直接コンタクトを促す UI。

提案（SPEC-015/016）→ 人の依頼・紹介 → **本 SPEC の `introductions`** という順で、**与える人に功績が残る**ことを SSOT で固定する。
...
- **121 起点の提案（SPEC-015）:** [ONETOONE_REFERRAL_SUGGESTION_REQUIREMENTS.md](ONETOONE_REFERRAL_SUGGESTION_REQUIREMENTS.md) — **MVP 実装済み（Phase 190–192）**。1 to 1 一覧「リファーラル」→ 提案モーダル → `register-introduction`。理念拡張は COMMON §0 Phase F（195）。
- **定例会起点の提案（SPEC-016）:** [CHAPTER_MEETING_REFERRAL_SUGGESTION_REQUIREMENTS.md](CHAPTER_MEETING_REFERRAL_SUGGESTION_REQUIREMENTS.md) — **MVP 実装済み（Phase 190–192）**。Meetings 入口・`meeting_id` 付き紹介登録。共通: [REFERRAL_SUGGESTION_COMMON.md](REFERRAL_SUGGESTION_COMMON.md)（§0.8 きっかけ作り・§0.8.7 Givers Gain）。
...
- [REFERRAL_SUGGESTION_COMMON.md](REFERRAL_SUGGESTION_COMMON.md) — 提案機能族・§0.8.7 Givers Gain
...
| 日付 | 内容 |
|------|------|
| 2026-06-04 22:14 JST | §2.1 Givers Gain と提案の from/to（つなぎ手 A）。§10 SPEC-015/016 MVP 実装済みに更新。 |
| 2026-05-18 07:25 JST | 初版確定（Phase 122）。 |

...
| R2 | 一覧で「自チャプター相手のみ」等のフィルタ | **実装済み（Phase 303）**。`GET /api/one-to-ones` / `stats` に `cross_chapter=1|0`（`is_cross_chapter` と同定義）・`target_workspace_id`・`target_group_name`・`target_category_id` を追加し、一覧の常時表示フィルターバーから操作。 |
...
- **1 to 1 一覧フィルターバー（Phase 303 / 304）:** 検索（相手名・かな・メモ）／すべて・自チャプター・他チャプター トグル／相手チャプター（ワード検索 Autocomplete）／カテゴリ（ワード検索 Autocomplete。大カテゴリ名・カテゴリ名どちらでも絞れ、大カテゴリごとにグループ表示。「◯◯（大カテゴリすべて）」→ `target_group_name`、個別カテゴリ → `target_category_id`）／状態／期間／キャンセルを除く。適用中フィルターを Chip 表示・個別解除・クリア。一覧・統計カード・件数は同じ filter で連動。
...
| **検索・フィルタ** | 「他チャプターとの 1 to 1 のみ」一覧フィルタは **Phase 303 で実装**（`cross_chapter=1`）。 |
...
| 2026-09-02 10:05 JST | Phase 303: R2（一覧フィルタ）を実装済みに更新。`cross_chapter` / `target_workspace_id` / `target_group_name` / `target_category_id` と常時表示フィルターバーを記載。 |
| 2026-09-02 10:40 JST | Phase 304: §5.3 相手チャプター・カテゴリをワード検索 Autocomplete に（大カテゴリ／カテゴリを 1 欄に統合）。 |

...
| `bo_assigned` | **`bo_assignment_audit_logs`**（BO-AUDIT-P1）。`actor_owner_member_id` で Dashboard owner に一致する行のみ。 |

**`bo_assignment_audit_logs`:** BO1/BO2（または breakout-rounds）保存成功時に `BoAssignmentAuditLogWriter` が **1 行追加**。詳細は [BO_AUDIT_LOG_DESIGN.md](BO_AUDIT_LOG_DESIGN.md)。**`meeting_csv_apply_logs` とは別**（CSV 反映は BO 割当イベントではない）。

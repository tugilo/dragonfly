...
| 2 | いつ会ったか | meetings | 集会（定例会・チーム MTG 等）。回番号または種別+日付。 |
| 2a | 議事録 | meeting_minutes | 集会の全体記録（Markdown・file→DB）。SPEC-014 / SPEC-018。 |
...
Religo では BNI 正式用語に合わせて「1 to 1」を扱う。表記: UI/文章は "1 to 1"、テーブル名は one_to_ones、memo_type の値は one_to_one。1 to 1 は「予定」「実施」「キャンセル」を持つイベントとして扱う。**121 履歴は `one_to_ones` で 1回 = 1行。** 各行の `notes` には **当該回の記録のみ**を保存する。相手との **共有メモ**（プロフィール・サマリー・累積インサイト等）は `contact_memos`（`memo_type = one_to_one`, **`one_to_one_id` は任意・null でペア共通**）に置いてよい（SPEC-019 §4.5）。
...
| C | meetings | meetings | いつ会ったか。集会（種別・回番号・日付・team_id）。 |
| C0 | meeting_types | meeting_types | 集会種別マスタ（SPEC-018）。 |
| C1 | meeting_minutes | meeting_minutes | 議事録（Meeting 1:1・Markdown 本文）。SPEC-014 / SPEC-018。 |
...
- **one_to_ones 1:N contact_memos**  
  当該回の主たる記録は **`one_to_ones.notes`** に書く（**1回1行**）。`contact_memos` は (1) **ペア共通**（`memo_type = one_to_one`, `one_to_one_id` null）— プロフィール・サマリー等の共有文脈、(2) **当該回への追記**（`one_to_one_id` 設定）— 時系列の分割メモ、の両方を許容する（SPEC-019 §4.5）。
...
### 4.6 meetings（Phase 220 拡張・SPEC-018 で meeting_types 連携予定）

| 項目 | 内容 |
|------|------|
| **目的** | いつ会ったか。**定例会**（番号付き）および **モメンタム/BOD/チーム MTG** 等の集会。 |
| **主キー** | id (bigIncrements) |
| **外部キー** | **meeting_type_id → meeting_types.id**（SPEC-018・未実装）。将来 workspace_id → workspaces.id を追加可。 |
| **ユニーク制約** | `number` UNIQUE（nullable）。番号なし: `(meeting_type_id, held_on)` またはチーム MTG は `(meeting_type_id, team_id, held_on)`（SPEC-018） |
| **主要カラム** | id, **meeting_type_id (FK, SPEC-018)**, number (unsignedInteger, nullable), **session_type (string, legacy 互換)**, **team_id (string, nullable, SPEC-018)**, held_on (date), name (nullable), timestamps |
| **インデックス** | held_on, number, (session_type, held_on), **(meeting_type_id, team_id, held_on)（SPEC-018）** |
...
**議事録（SPEC-014 / SPEC-018）:** 集会の全体記録は **`meeting_minutes`**（本 meeting と 1:1）。`contact_memos`（`memo_type=meeting`）の個人メモとは別。取り込み: `dragonfly:import-chapter-minutes`（chapter）、`dragonfly:import-team-minutes`（team・SPEC-018 未実装）。詳細は [CHAPTER_MINUTES_REQUIREMENTS.md](CHAPTER_MINUTES_REQUIREMENTS.md) / [TEAM_MEETING_MINUTES_REQUIREMENTS.md](TEAM_MEETING_MINUTES_REQUIREMENTS.md) / [MEETING_DOMAIN_IA.md](MEETING_DOMAIN_IA.md)。
...
### 4.6a meeting_minutes（SPEC-014・Phase 180 / SPEC-018 拡張）

| 項目 | 内容 |
|------|------|
| **目的** | **集会**の議事録（Markdown 全文）。`docs/meetings/chapter/` および `docs/meetings/team/` から file→DB 取り込み。会全体の記録（owner→target ではない）。 |
| **主キー** | id (bigIncrements) |
...
**doc_type 例:** `chapter_weekly`, `chapter_momentum`, `chapter_bod`, **`team_meeting`**（SPEC-018）

---

### 4.6b meeting_types（SPEC-018・未実装）

| 項目 | 内容 |
|------|------|
| **目的** | 集会種別マスタ。採番要否・参加者/BO/リファーラル UI 可否を定義。 |
| **主キー** | id (bigIncrements) |
| **外部キー** | なし |
| **ユニーク制約** | **code UNIQUE** |
| **主要カラム** | id, **code (string)**, **name_ja (string)**, **is_numbered (boolean)**, **requires_team_id (boolean)**, **supports_participants (boolean)**, **supports_breakouts (boolean)**, **supports_referral_suggestions (boolean)**, **sort_order (int)**, **is_active (boolean)**, timestamps |

**初期 code:** `chapter_weekly`, `momentum_training`, `business_open_day`, `team_meeting`, `webmaster_meeting`（後者は seed のみ先行）。

詳細: [TEAM_MEETING_MINUTES_REQUIREMENTS.md](TEAM_MEETING_MINUTES_REQUIREMENTS.md) §4
...
**notes の位置づけ（実装・UX・ONETOONES-P3・補足）:** `one_to_ones` は **1 行が 1 回の 1 to 1**（同一オーナー×ターゲットでも予定・実施が複数行あり得る）。`one_to_ones.notes` には **その回で話した内容**（議題・決定・次アクションを含めてよい）のみを保存する。**回をまたぐ文脈**（プロフィール・サマリー・累積インサイト等）は **各回の `notes` に重複コピーしない。** 共有文脈は `contact_memos`（`memo_type = one_to_one`, **`one_to_one_id` nullable = ペア共通メモ**）に置いてよい（SPEC-019 §4.5）。当該回への追記だけを時系列に残す場合は `one_to_one_id` を付けて `contact_memos` を併用する（ONETOONES-P4）。

**Markdown 取り込み（docs/meetings/1to1/）:** 運用は **1相手1ファイル**・本文は `### 【第N回】` で複数回を管理（[meetings/1to1/README.md](../meetings/1to1/README.md)）。現行 `dragonfly:import-1to1-notes` は 1ファイル→1行・全文 `notes`。複数回のセクション単位取り込みは [ONETOONE_MULTI_SESSION_IMPORT_REQUIREMENTS.md](ONETOONE_MULTI_SESSION_IMPORT_REQUIREMENTS.md)（SPEC-019・未実装）。

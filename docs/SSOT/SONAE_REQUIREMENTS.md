...
**初期対象:** BNI DragonFly チャプター BCP（PoC / 実証実験）  
**初期費用方針:** 本PoCにおいて、DragonFlyへの開発費・利用料の請求は行わない。BCP担当者の負担軽減と tugilo のサービス化実証を目的に、使いながら育てる。
...
初期は DragonFly BCP メンバーに実際に使ってもらいながら改善する **PoC（Proof of Concept / 実証実験）** として開始する。DragonFly 専用の作り込みにはせず、BNI 他チャプター、小規模コミュニティ、遠隔地で離れて暮らす家族、仲間内でも使える形へ育てる。

DragonFly では開発費・利用料を前提にせず、Givers Gain の精神に沿って、BCP担当者の負担軽減と tugilo のサービス実証を両立させる。
...
- DragonFly は「発注元」ではなく、最初のモデルチャプターとして位置づける。
- 災害時の回答体験は迷わないことを優先し、入力項目を増やしすぎない。
- 管理画面は BCP 委員・役員が状況確認と訓練を回せる範囲に絞る。
- メール、SMS、自動再送、課金、複雑な権限管理は PoC では扱わない。
- 将来拡張に必要なデータ境界、通知境界、テナント境界は PoC から保持する。
...
### 2.6 Member Roster Policy（メンバーロスター）

機能拡張のたびに `*_members` テーブルを増やさない。**1 roster + N extensions** とする。

| レイヤ | 役割 |
|--------|------|
| **Roster** | チャプター内の安否確認対象者。SONAE 実行時の正は **`sonae_members`** |
| **Extension** | LINE 紐付け、将来の活動地域、外部アカウント等。`member_id` / `sonae_member_id` でぶら下げる |
| **Adapter** | Religo 利用時、`members` → `sonae_members` への供給 |

Religo 利用時:

- Religo `members`（`workspace` 単位）が供給元。同期対象は **`type=member` のみ**（guest / visitor 等は除外）。
- **`source_system=religo`** + **`external_id=members.id`** で 1:1 upsert（コピー同期）。参照のみにしない。
- Religo `members` に **LINE userId・活動市町村は持たない**（Religo コアを SONAE 依存にしない）。
- 同期タイミング（PoC）: 初回 bootstrap + 訓練発報前の手動同期。

SONAE 単体利用時:

- `sonae_members` が正。CSV / 手動登録。

通知・回答・集計・発報履歴のコードパスは、Religo 有無で分岐させず **`sonae_members` を常に参照**する。

### 2.7 実装 DB 命名（Religo 共存）

Religo 既存表（`members`, `users` 等）との衝突を避けるため、SONAE 実装テーブルは **`sonae_*` 接頭辞**を用いる。本書 §11 の論理名（`chapters`, `members`）は ER 上の概念名。物理名は `sonae_chapters`, `sonae_members` 等とする。
...
#### L1 — 初回訓練（気象庁自動発報なしでも達成可）

- DragonFly のメンバーを Religo 連携（`type=member` sync）または CSV で `sonae_members` に登録できる。
...
- 気象庁情報の手動取得・定期取得ができる（§9.3 の **9種すべて**）。
- 取得した防災情報から `AlertEvent` を生成できる（種別ごと Normalizer）。
- 管理画面の **発報条件設定**（地域・閾値・ON/OFF）に合致した場合、自動で LINE 発報できる。
- 発報履歴・エラーログを確認できる。
- 月1回程度の訓練を継続運用できる。

#### 共通

- PoC 期間中、**LINE 友だち登録かつ `sonae_line_user_links` で紐付け済みのメンバーのみ**を訓練・災害発報の通知対象とする（§5.5）。
- `sonae_members` には全 active メンバーを登録するが、未紐付け者には Push 通知を送らない。未紐付け一覧で登録率を可視化する。
- 地域別回答義務（必須/任意/対象外）は §14 将来拡張。
...
| 項目 | PoC 方針 |
|------|----------|
| 名簿（`sonae_members`） | Religo sync または CSV で **全 active メンバー** を登録 |
| Push 通知対象 | **`sonae_line_user_links` に有効な紐付けがあるメンバーのみ** |
| パイロットフラグ | **設けない**。登録率の自然増加で段階的に通知対象が広がる |
| 未紐付け者 | 管理画面で一覧表示。訓練前に LINE 友だち登録・紐付けを促す |
| 回答率の分母 | 通知送信対象（紐付け済み）人数。名簿全体人数は参考 KPI として併記可 |
| 自動発報 ON | **手動訓練で登録率が十分になってから** ON にすることを推奨 |

訓練発報・災害自動発報とも、通知作成時に `notification_targets` は **紐付け済みメンバー分のみ** 生成する。

### 5.6 DragonFly 導入ステップ（Runbook）

| Step | 内容 | 担当 |
|------|------|------|
| 1 | SONAE 管理画面で DragonFly チャプターを bootstrap（Religo 連携時は members sync） | tugilo / BCP |
| 2 | DragonFly 公式 LINE Messaging API 設定を登録 | BCP |
| 3 | メンバーへ LINE 友だち登録・紐付けを周知（未紐付け一覧で進捗確認） | BCP |
| 4 | **手動訓練発報** → 回答・集計・改善（L1 達成） | BCP + tugilo |
| 5 | 発報条件初期値を設定し、JMA 自動取得・自動発報を有効化（L2） | BCP + tugilo |
...
| 12 | 訓練履歴 | 訓練一覧、回答率、**前回訓練との回答率比較**、未回答者、改善メモ |
...
- 発報日時
- 対象地域
- LINE 紐付け率（名簿人数に対する紐付け済み割合）

### 6.4 集計定義

| 指標 | 定義 |
|------|------|
| **被害あり人数** | 安否回答が `minor_injury`（軽傷）以上（`minor_injury` / `serious_injury` / `evacuating` / `hard_to_answer`）のメンバー数 |
| **活動困難人数** | 活動状況が `difficult` のメンバー数 |
| **定例会参加困難人数** | 定例会参加可否が `cannot_attend` のメンバー数 |
| **回答率** | 回答済み人数 ÷ 通知送信対象人数（`notification_targets` 件数） |
| **訓練回答率比較** | 同一チャプターの直前訓練との回答率差分（±%）を訓練履歴に表示 |

訓練発報と災害発報は **同一の安否回答画面・集計 UI** を用いる。訓練は `training_event_id` で履歴を分離するのみ（UI は共通化）。
...
- 1000文字以内。
- 管理画面の回答集計詳細に表示する。
- 災害時に個人情報・位置情報を過度に求めない。PoC では GPS 取得はしない。
...
```mermaid
flowchart TD
    A[管理者が訓練発報を作成] --> B[LINE 紐付け済みメンバーを対象に確定]
    B --> C[LINE 通知を送信]
    C --> D[メンバーが回答]
    D --> E[回答率を集計]
    E --> F[訓練履歴として保存]
...
| 監視種別 | §9.3 の **9種すべて**（提案書と同一。地震のみ先行などの省略は行わない） |
| 取得ジョブ | **1 本**（`JmaFetchService`）。種別ごとにジョブを増やさない |
| 正規化 | 種別ごと **`AlertEventNormalizer` アダプタ** → 共通 `AlertEvent` / `alert_event_areas` |
| 代替ソース | PoC では DMDATA.JP 等の第三者配信は使わない（依存・コスト増を避ける） |

実装時は JMAXML 電文コード表・2026 年以降の警報体系整理後サンプル XML を参照する。取得アダプタで `AlertEvent` に正規化し、`severity_rank` を付与して §9.6 の閾値判定に用いる。
...
### 9.5 種別と JMAXML 電文（実装参照）

種別ごとに Normalizer が担当する主な電文・情報（詳細は実装 Phase で公式コード表と突合）:

| alert_type | 主な JMAXML / 情報 |
|------------|-------------------|
| `earthquake` | VXSE51 震度速報、VXSE52/53 震源震度、VXSE43 緊急地震速報（EEW） |
| `tsunami` | VTSE41 津波警報・注意報 |
| `heavy_rain` | 警報 XML（2026 体系整理後の大雨系） |
| `flood` | 洪水警報 |
| `landslide` | 土砂災害警戒情報 |
| `typhoon` | 台風情報、暴風警報等 |
| `heavy_snow` | 大雪警報 |
| `volcano` | 噴火警報・火山活動 |
| `nankai_trough` | 南海トラフ地震臨時情報 |

### 9.6 発報条件設定 UI（閾値の柔軟変更）

閾値・対象地域を **コードにハードコードせず**、管理画面「発報条件設定」からチャプター単位・種別単位で変更できること。

| 項目 | 内容 |
|------|------|
| 一覧 | 9種の ON/OFF・対象地域・現在の閾値 |
| 編集 | 有効/無効、対象都道府県（複数）、対象市区町村（任意）、種別固有閾値 |
| 閾値マスタ | `alert_threshold_options`（`code`, `label`, `severity_rank`）。UI 選択肢はマスタから生成 |
| 保存 | `chapter_alert_settings.threshold_code` に選択値を保持 |
| 判定 | `AlertEvent.severity_rank >= threshold.severity_rank` かつ地域マッチ |
| 反映 | 保存後、次回 JMA 取得・判定サイクルから有効 |
| 地震 EEW | 地震設定内フラグ（例: `include_eew`）または専用 threshold |

API 案: `GET/PUT /api/sonae/chapters/{id}/alert-settings`、`GET /api/sonae/alert-threshold-options`。

### 9.7 取得ログ

`sonae_jma_fetch_logs`（論理名 `jma_fetch_logs`）に以下を記録する。
...
- PoC では回答は LINE 内の入力ではなく、通知内の回答 URL から Web 画面で行う。
...
PoC では本人確認を重くしすぎない。ただし誤紐付け防止のため、CSV 取込時に個別招待トークンを発行し、可能な限りトークン方式を使う。
...
PoC では **活動都道府県・活動市区町村は持たない**（Religo `members` にも同カラム無し）。将来 §14 の地域別回答義務用に `activity_areas`（json）等を extension として追加する。

#### alert_threshold_options

発報条件 UI の閾値選択肢マスタ。種別ごとに UI が参照する。

| カラム | 型案 | 内容 |
|--------|------|------|
| id | bigint | 主キー |
| alert_type_id | bigint | alert_types 外部キー |
| code | string | `intensity_5_lower` 等。`chapter_alert_settings.threshold_code` が参照 |
| label | string | 表示名（震度5弱以上 等） |
| severity_rank | integer | 判定用順序。AlertEvent と比較 |
| sort_order | integer | UI 表示順 |
| is_active | boolean | 有効 |
| created_at / updated_at | timestamp | 作成・更新日時 |

#### member_contacts

通知先候補。PoC では LINE が中心だが、メール・SMS 拡張に備える。
...
| カラム | 型案 | 内容 |
|--------|------|------|
| response_requirement | enum nullable | `required` / `optional` / `excluded`。PoC 全員 `required` 相当 |
...
| 可用性 | PoC では高可用構成までは求めないが、ジョブ失敗時に再実行できること |
...
| **地域別回答義務** | メンバー活動地域と警報影響域の照合。対象エリア内は回答必須、区域外は通知省略または任意（§14.1） |

### 14.1 地域別回答義務（将来）

PoC では **LINE 紐付け済みメンバー全員** を **回答必須** として通知する（§5.5）。将来、次により「全員回答ではなく、対象エリアは必須・それ以外は不要/任意」を実現する。

| 要素 | PoC | 将来 |
|------|-----|------|
| `sonae_members.activity_areas` | なし | 活動都道府県・市区町村（json 可） |
| `notification_targets.response_requirement` | 全員 `required` 相当 | `required` / `optional` / `excluded` |
| 未回答一覧 | 全通知対象 | **`required` のみ** |
| 回答率 KPI | 全員ベース | 必須対象ベース |

判定: `AlertEvent` の `alert_event_areas` と `Member.activity_areas` を `GeoMatchService` で照合し、発報時に `response_requirement` を付与する。

---

## 15. DragonFly PoC と事業化方針

### 15.1 DragonFly PoC の前提

DragonFly 向け初期版は、開発費・利用料をいただく前提ではなく、BCP担当者の負担軽減と tugilo のサービス化実証を目的とした **PoC（Proof of Concept / 実証実験）** として進める。
...
| 領域 | 内容 |
|------|------|
| 要件・設計 | SONAE PoC 要件整理、DB 設計、画面構成整理 |
...
料金は現時点の仮説であり、DragonFly PoC の運用実績、LINE公式アカウントの料金体系、利用頻度を見て調整する。

### 15.5 推奨フェーズ

| フェーズ | 内容 | 目安 |
|----------|------|------|
| P0 | 要件確定（[壁打ち合意](SONAE_WALL_BOUNCE_DECISIONS.md)）、LINE 準備、発報条件初期値 | 事前準備 |
| P1 | SONAE core: DB・手動訓練・LINE・回答・集計（Religo なしでも L1 可） | PoC 前半 |
| P2 | Religo adapter: members sync・管理画面統合 | PoC 前半 |
| P3 | JMA 9種 Normalizer・発報条件 UI・自動発報 | PoC 中盤（L2） |
| P4 | DragonFly 初回訓練、改善反映、他チャプター展開メモ | PoC 後 |

---

## 16. リスクと確認事項

### 16.1 DragonFly BCP（飯田香さん）確認事項

| # | 項目 | 備考 |
|---|------|------|
| 1 | 公式 LINE Messaging API 利用可否 | Channel 開設・Webhook URL |
| 2 | 自動発報の初期閾値 | 例: 震度5弱以上 |
| 3 | 対象地域 | 静岡県全体 vs 市区町村単位 |
| 4 | 訓練頻度 | 月1回でよいか |
| 5 | 未回答フォロー担当 | BCP 内役割分担 |
| 6 | 集計閲覧者 | 役員・閲覧者ロール |
| 7 | 既存チームスレッド運用 | SONAE との並行/移行 |
| 8 | 災害時の手動本番発報 | 自動発報 OFF 時、管理者が手動で災害通知を送る要否（PoC では手動訓練で代替可） |
...
| LINE 友だち未登録メンバーに通知できない | 未紐付け一覧を可視化し、訓練前に登録率を上げる（§5.5） |
| 災害時に回答 URL を開けないメンバーがいる | PoC では代替回答登録を管理者が手動入力できる拡張余地を残す |
| 誤発報 | 発報条件と対象地域を明確化し、重複防止と履歴確認を実装する |
| PoCの範囲が膨らむ | 通知・回答・集計・訓練に集中し、課金や複雑な権限は後回しにする |
...
- Religo 利用時は認証・チャプター・メンバー情報を**供給源**とし、SONAE 実行時正は **`sonae_members`**。通知・回答・発報履歴は SONAE 側で保持する。
- Religo `members` に LINE ID や活動地域を載せない（§2.6）。
- SONAE 単体利用時は SONAE 側でチャプター・メンバー・ログインを保持する。
- 物理 DB 表は `sonae_*` 接頭辞（§2.7）。
- JMA は 1 取得ジョブ + 9種 Normalizer。閾値は管理画面 + `alert_threshold_options`（§9.6）。
- 初期 UI は実運用優先で簡潔にし、訓練後の改善を前提にする。
- 災害時の本番運用に関わるため、発報履歴、送信ログ、回答ログは必ず残す。
- 未回答集計は将来 `response_requirement` でフィルタ可能な service 設計とする（§14.1）。
- Push 通知対象は LINE 紐付け済みメンバーのみ（§5.5）。パイロットグループ用フラグは設けない。

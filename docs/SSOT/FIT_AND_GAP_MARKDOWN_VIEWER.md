...
**依存:** `react-markdown` ^10.1.0、`remark-gfm`（Phase 181）。
...
| Meetings Drawer **メモタブ** | `memo_body` | Markdown レンダリング | `MarkdownView` dense=false（Phase 182） |
...
| 1to1 Edit **履歴メモパネル** | `contact_memos.body` | Markdown レンダリング | `MarkdownView` dense（Phase 182） |
...
| MV04 | 例会メモの閲覧 | メモタブでも Markdown 対応（任意） | `MarkdownView`（Phase 182） | **Fit** | — |
| MV05 | 1to1 履歴メモの閲覧 | 履歴も Markdown レンダリング | `MarkdownView` dense（Phase 182） | **Fit** | — |
...
| 2 | **MARKDOWN_VIEWER_P2** | Meetings メモタブ・1to1 履歴メモを `MarkdownView` 化 | **完了（Phase 182）** |
| 3 | **MARKDOWN_VIEWER_P3**（任意） | 議事録内リンク方針 | 未着手 |

### DoD（P2 完了の目安）

- [x] Meetings Drawer メモタブが `MarkdownView` 表示
- [x] 1to1 Edit 履歴メモが `MarkdownView` 表示（編集 UI は textarea 維持）
- [x] `npm run build` 成功
- [x] 本 Fit/Gap の MV04 / MV05 を **Fit** に更新
...
| 日付 | 内容 |
|------|------|
| 2026-06-02 16:36 JST | Phase 182 実装後: MV04/MV05 を Fit に更新 |

...
| **認証情報（API キー等）の保管** | **OAuth アプリ資格情報（Client ID / Client Secret / Webhook Secret）はユーザーごとに `user_zoom_credentials` に暗号化保存**（Phase 189・AI BYO key と同型）。`client_secret` / `webhook_secret_token` は Eloquent `encrypted` キャスト。API には平文を返さない。**`ZOOM_REDIRECT_URI` はシステム共通**（全ユーザー同一 callback・`.env`）。`.env` の `ZOOM_CLIENT_ID` 等は移行期間のフォールバック。コード・リポジトリにハードコードしない。 |
| **トークン保管** | ユーザーごとに動的発行される **アクセス／リフレッシュトークンは `zoom_accounts` に暗号化保存**（`.env` ではなく DB）。 |

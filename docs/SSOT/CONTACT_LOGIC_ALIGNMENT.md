...
**関連:** [CONTACT_AND_LEAD_DATE_LOGIC_REVIEW.md](CONTACT_AND_LEAD_DATE_LOGIC_REVIEW.md)、[DASHBOARD_DATA_SSOT.md](DASHBOARD_DATA_SSOT.md)、[DATA_MODEL.md](DATA_MODEL.md) §4.12.1、`MemberSummaryQuery::batchContactBreakdown`、`DashboardService::getTasks`、`MemberOneToOneLeadService`。
...
`App\Queries\Religo\MemberSummaryQuery::batchContactBreakdown` により、チャネル別の最大値を求めたうえで、その **合成 MAX** が各 target ごとの `last_contact_at` となる（**Dashboard 未接触 KPI と同一のソース集合**。introduction は本クエリでは含めない）。
...
### 1.5 `summary_lite` のチャネル別タイムスタンプ（Phase 125）

`GET /api/dragonfly/members?owner_member_id=&with_summary=1` の各 `summary_lite` に、グローバル Owner 基準で **接触チャネルごとの最終日時**を含める（**既存テーブルからの派生**。別テーブルへのスナップショット保存はしない）。

| フィールド | 意味 |
|------------|------|
| `last_bo_contact_at` | 例会 **BO 同席**（`participant_breakout` 同一ルーム・`held_on`）の最新 |
| `last_one_to_one_contact_at` | **1 to 1**（canceled 以外・effective 日時）の最新 |
| `last_memo_contact_at` | **接触メモ**（`contact_memos.created_at`）の最新 |
| `last_contact_at` | **Members summary_lite では** BO・メモ・1to1 の実効日時の **MAX**（本クエリでは **introductions は合成に含めない**。DATA_MODEL の「紹介を含む」完全合成は別ペインディング）。 |

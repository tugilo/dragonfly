...
| Gap | 内容 | 影響 | 推奨 | 優先度 |
|-----|------|------|------|--------|
| ~~G1~~（解消） | ~~Members 詳細ドロワーから email を直接編集できない~~ → **Phase 270 で解消**。ドロワー Overview に admin 限定 email 編集カードを追加 | — | 完了 | Done |
...
| 2026-06-27 13:05 | Phase 270 で G1 を解消（ドロワー Overview に admin 限定 email インライン編集を追加） |

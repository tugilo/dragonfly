...
|------|----------------------------|---------------------|
| ページヘッダー | 「Meetings」「例会管理 / BO割当 / メモ」 | タイトル＋サブ説明（M1 実装済み） |
| ヘッダーアクション | 「Connectionsで編集」 | 同一 |
| 統計カード | 総例会数 / 総BO数 / メモ有り例会 / 次回例会 | 4 種表示（M6 実装済み） |
| 一覧 | 番号・日付・BO数・メモ・Actions（📝メモ、🗺BO編集） | 番号・開催日・BO数・メモ・Actions（M1/M2 実装済み） |
| 例会詳細 | 右パネル or Drawer（番号・日付・BO数・メモ有無・BO割当・メモ編集・Connectionsへ） | 行クリックで Drawer が開き、同内容を表示（M3）。例会メモ編集は Dialog（M4） |

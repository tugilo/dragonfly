...
**整合ルール:** **interested または want_1on1 を変更したとき**は、**必ず contact_events に 1 件追加** する。理由（reason）は任意（空でも可）。**extra_status のみの変更では contact_events には追加しない**（V1 決定: [D-02](../../decisions/dragonfly/DRAGONFLY_DECISIONS_V1.md)）。これにより「いつ・なぜ」が追える。
...
- **Resolved（V1 決定）**  
  - extra_status のみ更新時の contact_events: 追加しない（[D-02](../../decisions/dragonfly/DRAGONFLY_DECISIONS_V1.md)）。owner の渡し方: 画面で owner_member_id を選択して API に渡す（[D-01](../../decisions/dragonfly/DRAGONFLY_DECISIONS_V1.md)）。その他 D-03〜D-08 は [DRAGONFLY_DECISIONS_V1.md](../../decisions/dragonfly/DRAGONFLY_DECISIONS_V1.md) 参照。

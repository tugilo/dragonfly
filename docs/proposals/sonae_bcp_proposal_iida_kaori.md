...
まずは tugilo の **PoC（Proof of Concept / 実証実験）** として、DragonFly を最初のモデルチャプターにし、必要最小限の機能で実際に動かしながら育てていく提案です。
...
DragonFlyでまず実運用してみるため、PoCでは必要最小限で現場で使える初期版に絞ります。
...
今回は tugilo の **PoC（実証実験）** として進めます。

DragonFly は最初のモデルチャプターとして、現場で使いながら一緒に育てていく役割をお願いしたいです。
...
本PoCにおいて、DragonFlyへの開発費・利用料のご請求はありません。  
ただし、DragonFly公式LINE側の通数・プラン費用が発生する場合は別途確認が必要です。

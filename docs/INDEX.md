...
| [MEETING_DOMAIN_IA.md](SSOT/MEETING_DOMAIN_IA.md) | 定例会ドメイン IA。Meeting=集約ルート、Connections=BO 編集面、Meetings=管理ハブ（タブ構成）。SPEC-014 関連。 |
...
| [FIT_AND_GAP_MARKDOWN_VIEWER.md](SSOT/FIT_AND_GAP_MARKDOWN_VIEWER.md) | **Markdown ビューア**（議事録・1to1 notes）の Fit/Gap。`MarkdownView`・GFM 表・画面別 pre-wrap 残存。Phase 180 後の改善候補（remark-gfm 等）。 |

...
| [meetings/1to1/1to1_nishiura_miyabi_draci.md](meetings/1to1/1to1_nishiura_miyabi_draci.md) | **西浦雅（BNI DragonFly／外構SNS集客）**。**Draci（WordPress）** 案件。**第1回 2026-06-04 JST 15:00–16:00 実施済み**・**Religo `one_to_ones.id=66`**。Zoom要約反映。**3層アカウント管理**（平岡氏＝ログイン専用／システム管理1アカウント／通常メンバー）、掲示板テストサーバー公開・支部検索来週実装・フォント変更。追加費用なし合意。 |
...
| [ONETOONE_REFERRAL_SUGGESTION_REQUIREMENTS.md](SSOT/ONETOONE_REFERRAL_SUGGESTION_REQUIREMENTS.md) | **SPEC-015** — 1 to 1 **実施後リファーラル提案**。一覧に「リファーラル」ボタン→提案モーダル。生成ごとに **run** を保存（notes 版の指紋付き）。採用・却下の履歴と **`introductions` へのつなぎリンク**。**Phase 190–191 MVP 実装済み**（develop）。 |
| [CHAPTER_MEETING_REFERRAL_SUGGESTION_REQUIREMENTS.md](SSOT/CHAPTER_MEETING_REFERRAL_SUGGESTION_REQUIREMENTS.md) | **SPEC-016** — **定例会議事録**（**メインプレ**・ウィークリー・ビジター等）からのリファーラル提案。Meetings 入口・`body_digest` run・採用時 **`introductions.meeting_id`**。**Phase 190–191 MVP 実装済み**（develop）。 |
| [REFERRAL_SUGGESTION_COMMON.md](SSOT/REFERRAL_SUGGESTION_COMMON.md) | **リファーラル提案 共通 SSOT**（SPEC-015/016 横断）。run・digest・採用フロー・introductions マッピング。**実装 Phase 190〜194** → [ロードマップ](process/phases/REFERRAL_SUGGESTION_PHASE_ROADMAP.md)。 |
...
| [phases/REFERRAL_SUGGESTION_PHASE_ROADMAP.md](process/phases/REFERRAL_SUGGESTION_PHASE_ROADMAP.md) | **リファーラル提案 実装ロードマップ**（SPEC-015/016・Phase 190〜194 対応表・受入データ）。 |

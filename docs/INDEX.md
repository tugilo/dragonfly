...
| [FIT_AND_GAP_MENU_HEADER.md](SSOT/FIT_AND_GAP_MENU_HEADER.md) | メニュー・ヘッダーに特化したフィット＆ギャップ（モック v2 基準の調査）。実装時の参照用。 |
...
| [USER_ME_AND_ACTOR_RESOLUTION.md](SSOT/USER_ME_AND_ACTOR_RESOLUTION.md) | `/api/users/me`・BO 監査 actor・workspace（chapter）解決の SSOT（BO-AUDIT-P3〜P4）。 |
| [WORKSPACE_RESOLUTION_POLICY.md](SSOT/WORKSPACE_RESOLUTION_POLICY.md) | `workspace_id` 解決順（default → owner 由来データ → 先頭 workspace）の SSOT（BO-AUDIT-P4）。 |

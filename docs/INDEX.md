...
| [ZOOM_ONETOONE_SYNC_REQUIREMENTS.md](SSOT/ZOOM_ONETOONE_SYNC_REQUIREMENTS.md) | **SPEC-012** — Zoom 連携による **1 to 1 の予定起票・実績時刻反映・要約取得・相手の手動正規化**の要件整理。Zoom を正の情報源に二重入力と時刻 TODO を削減。相手未登録は M7/M8（CSV 取込）・SPEC-008 を流用し人の確認必須。カレンダー連携は当面非採用。段階1（読取取込）→2（要約）→3（Webhook）。**draft**／記録: Phase 151。 |

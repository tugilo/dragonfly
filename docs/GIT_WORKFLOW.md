...
- feature ブランチ名は Phase や目的が分かるようにする（例: `feature/phase126-auth-spec010-owner-binding`）。
- **1 Phase ごとに:** `feature/***` をローカルで `develop` に **merge（--no-ff）** したら、**続けて同一作業のうちに `git push origin develop`** まで行い、**リモートの統合ブランチを更新する**（フェーズをまたいで `develop` が `origin/develop` より先行したまま滞留させない）。
...
- オフラインなどで push できない場合は `docs/dragonfly_progress.md` か REPORT に **未 push TODO** を明記し、復旧後 **最優先で `git push origin develop`** する。
